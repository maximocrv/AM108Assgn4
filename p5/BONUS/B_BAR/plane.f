...
c
      ...
      DIMENSION Bv_bar(4,18), Bv(4,18), B_BAR(4,18)
C
...
C---- INITALIZE Bv and B_BAR
      ...
      DO I = 1,4
          DO J = 1, NST
              B_BAR(I,J) = B(I,J) + Bv_bar(I,J) - Bv(I,J)
          END DO
      END DO
...
C---- FROM THE CURRENT GAUSS POINT L.
      ...
      do i = 1, NST           ! Loop over rows of S
          do j = 1, NST       ! Loop over columns of S
             ...
             S(i,j)=S(i,j)+XSJ*XJC*WG(L)*B_BAR(k,i)*DMAT(k,m)*B_BAR(m,j)
                  end do
              end do
          end do
      end do
...
C---- DUE TO ELEMENT STRESSES AT CURRENT GAUSS POINT L           
      ...
      DO i = 1, 2 * NEL
          DO j = 1, NS
              P(i) = P(i) - XSJ * XJC * WG(L) * B_BAR(j,i) * STRL(j,L)
          END DO
      END DO
...
C---- NOTE: LL = 1 to 9 correspond to 3x3 GP and  LL = 10 to 1x1 GP if NEL>4.
      ...
      DO I = 1,NEL
          DO J = 1,4
      EPSL(J,LL)=EPSL(J,LL)+B_BAR(J,2*I-1)*UL(1,I)+B_BAR(J,2*I)*UL(2,I)
          END DO
      END DO
